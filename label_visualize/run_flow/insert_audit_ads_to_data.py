...
def InsertContentAds(cursor, ads, d):
	...
	INDEX_CONTENT, SNAPSHOT_DATE, INSERT_DATE, FLAG, STATUS) \
	...
	if ads['list_product'] != [] and 'audit_content' in ads:
		...
		if list_thumbnail != []:
			for i, thumbnail in enumerate(list_thumbnail):
				...
				datetime.strptime(d, '%Y-%m-%d'), datetime.strptime((time.strftime('%Y-%m-%d')), '%Y-%m-%d').date(), 0, 'True'))
		...
		if video_ids != [] and 'object_story_spec' in ads:
			for i, video_id in enumerate(video_ids):
				...
				cursor.execute(statement, (ads['ad_id'], ads['list_product'][0], link, 'video_url', 0, i,  \
