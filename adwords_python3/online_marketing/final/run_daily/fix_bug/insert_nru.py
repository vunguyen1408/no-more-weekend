...
def Read_NRU(cursor, start_date, end_date, product):
	#==================== Get NRU =============================
...
def Add_NRU_into_plan(connect, path_data, date):
	...
	for plan in data['plan']:
		...
		data['plan'][data['plan'].index(plan)]['CCD_NRU'] = Read_NRU(cursor, ConvertDate(start_date), ConvertDate(end_date), plan['PRODUCT'])
...
def Add_NRU_into_list(connect, list_plan, date):	
	...
	for plan in list_plan:
		start_date, end_date = ChooseTime(plan)
		list_plan[list_plan.index(plan)]['CCD_NRU'] = Read_NRU(cursor, ConvertDate(start_date), ConvertDate(end_date), plan['PRODUCT'])
	...
	for plan in list_plan['TOTAL']:
		...
		if ('MONTHLY' in plan):		
			...
			for i in range(len(plan['MONTHLY'])):
				...
				list_plan['TOTAL'][list_plan['TOTAL'].index(plan)]['MONTHLY'][i]['CCD_NRU'] = Read_NRU(cursor, start_date, end_date, plan['PRODUCT'])
	...
	for plan in list_plan['UN_PLAN']:
		...
		if ('MONTHLY' in plan):
			...
			for i in range(len(plan['MONTHLY'])):
				start_date = ConvertDate(plan['MONTHLY'][i]['START_DATE'])
				end_date = ConvertDate(plan['MONTHLY'][i]['END_DATE'])
				list_plan['UN_PLAN'][list_plan['UN_PLAN'].index(plan)]['MONTHLY'][i]['CCD_NRU'] = Read_NRU(cursor, start_date, end_date, plan['PRODUCT'])
...
def AddNRU(connect, list_plan, date):
	...
	for plan in list_plan['TOTAL']:
		start_date, end_date = ChooseTime(plan)
		plan['CCD_NRU'] = Read_NRU(cursor, ConvertDate(start_date), ConvertDate(end_date), plan['PRODUCT'])
		if ('MONTHLY' in plan):		
			...
			for i in range(len(plan['MONTHLY'])):
				...
				plan['MONTHLY'][i]['CCD_NRU'] = Read_NRU(cursor, start, end, plan['PRODUCT'])
	cursor.close()
